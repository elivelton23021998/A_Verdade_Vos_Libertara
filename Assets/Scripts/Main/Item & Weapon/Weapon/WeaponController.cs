...
public class WeaponController : MonoBehaviour, ISwitcher, ISwitcherWallHit
{
    [HideInInspector]
	...
	[Header("Configurações de Inventário de Armas")]
	public int weaponID;
	public int bulletsID;

    [Header("Balas da Armas")]
    [Tooltip("Balas na Magazine")]
    public int bulletsRemaining = 0;

    [Tooltip("Balas por Magazine")]
    public int bulletsPerMag = 20;

    [Tooltip("Balas Atuais na Arma")]
    ...
    [Space(3)]
	[Tooltip("O elemento '0' é sempre o buraco de bala padrão.")]
    ...
    void Update()
    {
        ...
        {
           // FireKey = inputManager.GetInput("Atirar");
            ReloadKey = inputManager.GetInput("Recarregar");
            ZoomKey = inputManager.GetInput("Zoom");
        }

        if (inventory && weaponID != -1 && bulletsID != -1)
        {
            if (inventory.CheckItemInventory(weaponID))
            {
                inventory.SetItemAmount(weaponID, bulletsRemaining);
            }

            if (inventory.CheckItemInventory(bulletsID))
            {
	...
	IEnumerator DeselectEvent()
    {
        ...
        showUI = false;
        selected = false;
        playerFunctions.zoomEnabled = true;
        transform.GetChild(0).localPosition = hipPosition;
