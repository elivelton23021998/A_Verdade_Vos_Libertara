...
/// <summary>
/// Script que controla as opções em tempo de execução
/// </summary>
public class UICustomOptions : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        {
            Debug.LogError("Options Error: UICustomOptions não funciona sem o script ConfigHandler e InputController!");
        }
        ...
        {
            Debug.LogError("PostProcessing Error: Não há script PostProcessingBehaviour adicionado a uma câmera principal!");
        }
    }
    ...
    void Start()
    {
        ...
        SetResolutions();
        SetQualities();
        UpdateOptions(!GetComponent<AVVL_GameManager>());
    ...
    private void ApplyGraphic(bool Serialize)
    {
        ...
        if (bwChanged)
        {
            QualitySettings.skinWeights = (SkinWeights)ConvertBWLevel(BlendWeightsLevel);
            bwChanged = false;
        }
        if (vsyncChanged)
        {
    ...
    private void ApplyControls()
    {
        ...
        {
            Debug.LogError("Options Error: O script InputController não foi encontrado!");
        }
    }
