...
/// <summary>
/// Script que trata de todas as ações de configuração.
/// </summary>
public class ConfigHandler : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        if (!GetComponent<AVVL_GameManager>())
        {
            ...
            {
                Debug.Log("<color=yellow>Config Warning: " + ConfigName + " não existe na pasta Dados! A configuração será criada automaticamente.</ color>");
                Error = true;
            }
        }
        else
        {
            ...
            if (!ConfigManager.ExistFile(ConfigName))
            {
                Debug.LogError("Config Error: " + ConfigName + " não existe na pasta Dados!");
                Error = true;
            }
        }
