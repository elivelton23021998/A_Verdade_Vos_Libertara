...
/// <summary>
/// Fornece métodos para arquivos de configuração Serialize e Deserialize
/// </summary>
public static class ConfigManager
{
    ...
    public static void SetFilename(string Filename)
    {
        if (!pathSet)
        {
            folderPath = GetFolderPath(FilePath.GameDataPath);
        }
    ...
    public static async void Serialize(string Section, Dictionary<string, string> keyValuePairs)
    {
        ...
        else if (enableDebug) { Debug.Log("<color=yellow>Dictionary está Vazio!</color>"); }
    }
    ...
    public static string Deserialize(string Section, string Key)
    {
        if (ConfigDictionary.ContainsKey(Section))
        {
            ...
            else
            {
                if (enableDebug)
                {
                    Debug.Log("<color=yellow>Nenhuma chave encontrada nesta seção</ color>");
                }
                return null;
            }
        }
        ...
        {
            if (enableDebug) { Debug.Log("<color=yellow>Nenhuma seção com este nome encontrada</ color>"); }
            return null;
        }
    }

    /// <summary>
    /// Este método funciona apenas com tipos de sistema
    /// </summary>
    ...
    {
        string pathfile = GetFolderPath(filePath) + file + ".cfg";
        if (File.Exists(pathfile))
        {
            File.Delete(pathfile);
            if (enableDebug) { Debug.Log("<color=red>Config File Removido:</color> " + pathfile); }
        }
        else
        {
            if (enableDebug) { Debug.Log("<color=yellow>Config File Não Encontrado:</color> " + pathfile); }
        }
    }

    public static void DuplicateFile(FilePath filePath, string file, string duplicate)
    {
        string pathfile = GetFolderPath(filePath) + CheckExtension(file);
        if (File.Exists(pathfile))
        {
            File.Copy(pathfile, GetFolderPath(filePath) + duplicate + ".cfg");
        }
        else
        {
            if (enableDebug) { Debug.Log("<color=yellow>Config File Não Encontrado:</color> " + pathfile); }
    ...
    public static string GetFolderPath(FilePath Filepath)
    {
    ...
    public static string GetFilepathRoot(FilePath Filepath)
    {
        if (Filepath == FilePath.GameDataPath)
        {
    ...
    {
        return GetFolderPath(filePath) + CheckExtension(file);
    }

    public static string GetFileAndPathFolder(FilePath filePath, string folder, string file)
    {
        return GetFilepathRoot(filePath) + "/" + folder + "/" + CheckExtension(file);
    }
