...
public class DynamicObject : MonoBehaviour, ISaveable {
    ...
    [Tooltip("Se o vetor de movimento padrão verdadeiro for X, se o vetor padrão falso for Z")]
    ...
    void Awake()
    {
        if(dynamicType == Type_Dynamic.Door)
        {
            if(interactType == Type_Interact.Mouse)
            {
                ...
                {                   
                    Debug.LogError(transform.parent.gameObject.name + " requer Componente de Hinge Joint!");
                    return;
                }
            }
    ...
    private void TryUnlock()
    {
        if (keyType == Type_Key.Inventory)
        {
            ...
            {
                Debug.LogError("O script de inventário não está definido!");
            }
        }
        else
        {
